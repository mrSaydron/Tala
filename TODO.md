...
Реализовать минимальную функциональность для версии с разделенными сущностями

### В активной работе:

    Заполнять таблицу с прогрессом сразу при создании урока (пока для всех слов без фильров)
    Добавить методы для работы с прогрессом слов: смена статуса, вычисление следующей даты изучения. 10 градаций для прогресса
Добавить/проверить метод для формирования списка изучаемых карточек. Пока просто выбираются все по дате
Добавить/проверить метод для изменения позиции карточки в списке текущего урока в зависимости от ответа
    Поправить фрагмент прямого перевода. Две стороны у карточки, кнопки для оценки знания, изображение
    Добавить фрагмент обратного перевода
    Добавить фрагмент ввода слова
    Добавить фрагмент и логику для типа карточки со связыванием слова и его перевода
    Добавить таблицу истории ответов
    Добавить сохранение истории в момент ответа пользователя
    Добавить таблицу настройки условий перехода между типами карточек
Добавить фрагмент для настройки условий перехода между типами карточек

### Баги и улучшения

Удаление уроков
Удаление коллекций
Удалить старые сущности
Удалить старые фрагменты
...
dictionary переименовать в word
