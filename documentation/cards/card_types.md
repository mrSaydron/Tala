...
```CardTypeEnum.TRANSLATION_COMPARISON```

Карточка показывает сразу несколько слов (от 2 до 5) и их переводы. Пользователь должен выстроить пары «слово → перевод», переставляя элементы в правой колонке. После нажатия на кнопку «Показать перевод» карточка фиксируется и отображает правильные сопоставления:
...
Если допущена хотя бы одна ошибка, карточка остаётся в текущей сессии: сервис возвращает обновлённую карточку с теми же `progressId`, чтобы пользователь повторил попытку. Каждый прогресс в карточке пересчитывается отдельно по SM-2: верно сопоставленные слова переводятся в состояние `IN_PROGRESS`, ошибочные переходят в `PROGRESS_RESET` и планируются на ближайшие 10 минут. При полностью верном ответе карточка больше не показывается в текущем уроке.
...
Карточка реализуется фрагментом `TranslationComparisonCardTypeFragment`, DTO — `TranslationComparisonLessonCardDto`, сервис — `TranslationComparisonLessonCardTypeService`.
