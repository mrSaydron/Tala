...
| Связанные документы | [Типы карточек](./card_types.md), [ER-диаграмма](../data/er_diagram.puml) |
...
- `word_id` — идентификатор изучаемого слова;
...
Метод `LessonCardService.answerResult` принимает DTO карточки, качество ответа и (при необходимости) пользовательский ввод, после чего делегирует вызов конкретному `LessonCardTypeService`. Реализация типа возвращает:

- `null`, если карточка успешно завершена и в текущей сессии больше не нужна;
- обновлённую DTO, если карточку требуется показать повторно (например, при ошибке).

Для базовых типов (`TRANSLATE`, `REVERSE_TRANSLATE`, `ENTER_WORD`) сервис обновляет один экземпляр `LessonProgress`. Для `TRANSLATION_COMPARISON` используется список прогрессов: каждая пара «слово → перевод» получает собственное обновление расписания, и если есть ошибки — карточка пересобирается с теми же `progressId`, чтобы пользователь повторил сопоставление.

Для аналитики и отображения статусов в UI `LessonCardService` предоставляет хелперы `countCardsByStatus(cards: List<LessonCardDto>)` и `countCardsByStatus(lessonId: Int)`. Первый подсчитывает количество карточек в каждом статусе в переданном наборе, второй использует `getCards(lessonId)`, чтобы вернуть актуальную статистику по уроку.
